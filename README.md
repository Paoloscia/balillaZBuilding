...
- **Suggeritore (`scorePairing`)**: novità (incroci mai visti x30, coppie mai viste x20), complementarità attaccante+difensore x8, tieBreak per scontri mai giocati tra ELO vicini (prossimità lineare su 60 punti, peso maggiore al vertice, x12), malus abitudine x6, attività, equilibrio. Ruoli: mai due attaccanti nella stessa coppia (fallback con avviso se nessuna combinazione li rispetta).
...
- **Storico**: pending in cima con conferma/rifiuta (l'ELO si calcola alla CONFERMA coi rating correnti, il repetitionFactor sul created_at), filtro per stagione, undo dell'ultima confermata (i delta interi si sottraggono esattamente), badge rosso in nav. Il vincitore ha il trofeo davanti al nome e il perdente è attenuato (classe `.loser`). Gli admin possono **correggere le formazioni** di una partita (matita): per le pending aggiorna solo le squadre, per le confermate della stagione attiva aggiorna squadre e winners e poi lancia automaticamente il ricalcolo completo della stagione. Le partite di stagioni chiuse non si correggono.
