...
- **Stagioni**: crea/pianifica (con ends_at facoltativa e countdown nell'header), coda numerata in ordine di attivazione, chiusura = premi in `awards` (il premio Coppia d'oro richiede gli stessi requisiti PAIR_QUAL delle migliori coppie: 10 partite insieme contro 6 coppie avversarie diverse) + snapshot in `season_results` + reset a scaglioni + avvio automatico della pianificata più vecchia (il client prova PRIMA la RPC `close_season(p_awards)` atomica di supabase-chiusura-backup.sql e solo se manca usa i passi dal client), rinomina, **ricalcolo stagione** (rigioca TUTTE le confermate in ordine di confirmed_at; punto di partenza = `pre` della prima partita di ciascuno; riscrive deltas/pre/winners e i rating; idempotente; usa i boost salvati).
...
- **Numeri**: in cima la card "La serata di oggi" (spostata qui dalla Classifica). Poi i record di sempre su TUTTE le partite confermate (fetch paginato on-demand alla prima apertura del tab, cache di sessione in `state.numeri`, bottone ↻ per ricalcolare): tutti i cappotti (totale + top 3 di chi li rifila e chi li subisce; cappotto = vittoria a 0 con almeno 8 gol), striscia piu lunga, piu tornei vinti (da state.tournaments, non dalle partite), winrate piu alto tra chi ha 100+ partite, rivalita piu giocata, coppia piu rodata, giorno piu intenso, e la sezione MIGLIORI COPPIE di sempre (spostata qui dalla Classifica): elenca TUTTE le coppie qualificate, cioe con almeno PAIR_QUAL.games=10 partite insieme contro PAIR_QUAL.opps=6 COPPIE AVVERSARIE diverse (non giocatori singoli), ordinate per winrate, senza limite di 5. RIMOSSI su richiesta di Paolo: vittoria piu larga, partita che ha spostato piu punti, upset piu grande.
- **ELO**: scheda esplicativa completa delle regole, in italiano, per i giocatori.

### Finale a 4, play-in e campione di stagione (scelta di design importante)

Il CAMPIONE della stagione NON e il primo della classifica ELO: la classifica serve solo a QUALIFICARSI. A stagione chiusa il percorso e in DUE passi:
1. **Play-in (4°-7°)**: i primi 3 dello snapshot vanno in finale di diritto; dal 4° al 7° si giocano l'ultimo posto con lo stesso formato a coppie rotanti. Scelta voluta per dare una porta d'ingresso a chi gioca meno. Formato playin4, bracket con campo extra `protected: [3 id]`. Serve avere 7 disponibili; se il gruppo e piccolo c'e il bottone "Salta il play-in: finale diretta coi primi 4".
2. **Finale a 4**: i 3 protetti + il vincitore del play-in giocano a coppie rotanti: 3 partite fisse con tutte le combinazioni (1+4 vs 2+3, 1+3 vs 2+4, 1+2 vs 3+4).

**Seeding (`seedOrder`)**: dallo snapshot, ordinato per ELO; a parita di ELO contano gli scontri diretti di stagione tra i due (h2h, calcolato sulla finestra delle 600: per stagioni lunghissime puo essere parziale), poi il win rate, poi il rank originale.

**Sostituzioni per la finale (`finaleCandidates`)**: protetti -> vincitore play-in -> altri del play-in nell'ordine della LORO classifica di play-in -> resto della graduatoria. L'admin segna gli assenti coi chip (state.f4Excl) in entrambi i passi e subentrano i successivi. Campione = piu VITTORIE nelle 3 partite; a parita: differenza reti, poi spareggio 1v1 secco (il client lo propone da solo quando serve). I gol totali NON sono un criterio, vengono solo mostrati nella tabella. NOTA STORICA: la prima idea era "chi segna piu gol", poi Paolo ha corretto esplicitamente in vittorie -> diff reti -> spareggio.

Dettagli implementativi:
- Finale e play-in vivono ciascuno in una riga di `tournaments` con `format: "finale4"` / `"playin4"`, `rated: false`, `teams: []` e un bracket custom `{players: [4 id in ordine di rank], matches: [{a:[i,i], b:[i,i], score}], tiebreak: {a, b, score} | null}`. NIENTE nuove tabelle o colonne.
- Le 3 partite NON toccano l'ELO e NON entrano in `matches`: vivono solo nel bracket. Si giocano tipicamente a stagione nuova gia in corso, per questo `recordTournamentMatch` dirotta subito su `recordFinale` SALTANDO il check di stagione attiva.
- `tournamentWinnerIdx` ritorna null per finale4: il campione si legge con `finaleChampion(t)` che ritorna {status: playing|tiebreak|done, ...}.
- UI nel tab Stagioni, dentro la card della stagione chiusa nell'albo d'oro: la sezione finale sta SOPRA la regular season (la corona e il titolo vero). Se la finale non esiste ancora, SOLO sull'ultima stagione chiusa l'admin vede i chip dei primi 8 dello snapshot: tocca un giocatore per segnarlo ASSENTE (state.f4Excl, gestione a mano voluta da Paolo) e subentrano i successivi in classifica; giocano i primi 4 disponibili. Bottone "Lancia la finale a 4" -> createFinale.
- Finale e play-in NON compaiono nel tab Tornei (filtrati via notF4); `tournamentWinnerIdx` ritorna null per entrambi, quindi non contano come "tornei vinti" nei badge e nei Numeri.
- **Badge profilo** (spiegati anche nella card "I BADGE DEL PROFILO" del tab ELO): 🧹 cappotti, ⚔️ ammazzagiganti (+120 elo medio), 🔥 striscia >=5, 🥇🥈🥉 regular season chiuse 1°/2°/3°, 👑 campione (finale4 vinte), 🏅 "Qualificato alle finali" (in play-in o finale di una stagione con finale CONCLUSA, senza vincere il titolo), 🏆 tornei normali vinti. Il 🥄 cucchiaio di legno e stato RIMOSSO su richiesta di Paolo, sostituito dal 🏅.
- **Colori classifica** (`rankClass`, usato anche nella classifica completa dell'albo): 1° oro, 2° argento (#C9CDD3), 3° bronzo (#CD8A4F), 4°-7° numero a solo contorno marrone (#9A6B45, -webkit-text-stroke con fallback a colore pieno).
- La card della stagione chiusa mostra, in ordine: titolo/finale/play-in (o lancio), REGULAR SEASON col podio, i 5 premi, e la CLASSIFICA COMPLETA dello snapshot (tutti i giocatori con rank colorato, record e ELO). La riga "e altri N giocatori" e stata sostituita dalla lista completa. Badge profilo: 👑 "Campione di stagione" (da finale4 done), il vecchio 👑 rank 1 e diventato 🥇 "Regular season vinte".
- Sopra la classifica del tab Classifica c'e una card fissa "COME SI DIVENTA CAMPIONE" che spiega il formato ai giocatori.
- Il tab Classifica del tab... la card della stagione chiusa mostra: finale (o lancio) -> etichetta REGULAR SEASON -> podio -> premi -> conteggio altri giocatori.
