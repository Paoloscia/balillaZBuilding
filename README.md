...
13. `supabase-sfide.sql` (tabella challenges per le sfide dirette; facoltativo, senza la tabella la sezione si nasconde da sola)
...
- **Partita**: 1v1/2v2 (2v2 preselezionato), punteggio con griglia 0-10 a tap singolo, modalità "solo chi ha vinto", previsione con delta stimati, "Chi c'è oggi" con chip presenti e tavoli suggeriti. Admin registra direttamente, anonimo propone (pending). In fondo la sezione **Sfide dirette** (se esiste la tabella challenges): chiunque lancia una sfida tra due giocatori; lo stato "saldata" NON e salvato ma calcolato dal client (prima partita confermata dopo il lancio coi due su sponde opposte, singolo o coppia); le saldate recenti (max 5) restano visibili; solo admin elimina.
...
- **Numeri**: record di sempre su TUTTE le partite confermate (fetch paginato on-demand alla prima apertura del tab, cache di sessione in `state.numeri`, bottone ↻ per ricalcolare): vittoria piu larga, striscia piu lunga, partita che ha spostato piu punti (somma dei delta positivi), upset (attesa minima del vincitore), rivalita piu giocata, coppia piu rodata, re del cappotto (vittorie a 0 con almeno 8 gol), giorno piu intenso.
